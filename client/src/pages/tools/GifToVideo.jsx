...
import { Camera, Upload, Download, Loader2, RefreshCw } from "lucide-react";
...
export default function GifToVideo() {
  ...
  const convertToVideo = async () => {
    ...
    try {
      ...
      // Execute conversion: covert GIF to MP4 (h264)
      await ffmpeg.exec([
        "-f",
        "gif",
        "-i",
        ...
        "yuv420p",
        "-vf",
        "scale=trunc(iw/2)*2:trunc(ih/2)*2", // Ensure divisible by 2 for H264
        "output.mp4",
      ]);
      ...
      console.error(error);
      alert("Error converting GIF to Video.");
    } finally {
      setConverting(false);
    }
  };
  ...
  return (
    ...
    >
      <div className="mb-6 flex flex-col items-center text-center">
        <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-orange-500 to-amber-500 shadow-lg shadow-orange-500/20 flex items-center justify-center mb-4">
        ...
        <p className="text-foreground/60 mt-2 text-sm sm:text-base">
          Convert animated GIFs into efficient MP4 videos locally.
        </p>
      </div>
      ...
      <div className="glass-card rounded-2xl p-6 sm:p-8 mb-6 shadow-xl border border-border">
        <div className="flex flex-col gap-6">
          {!file ? (
            <div
              className={`flex flex-col items-center justify-center border-2 border-dashed rounded-xl p-10 min-h-[300px] cursor-pointer transition-all ${
                dragOver
                  ...
                  : "border-border hover:border-orange-500/50 hover:bg-white/5"
              }`}
              onDragOver={(e) => {
                e.preventDefault();
                setDragOver(true);
            ...
            >
              <Upload className="w-12 h-12 mx-auto mb-4 text-orange-500/60" />
          ...
          <button
            ...
            className="w-full mt-2 h-14 flex items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-orange-600 to-amber-500 text-white font-bold text-lg shadow-lg shadow-orange-500/20 hover:shadow-orange-500/40 transition-all disabled:opacity-50 disabled:cursor-not-allowed overflow-hidden relative"
          >
            {converting && (
              <div
            ...
            <span className="relative z-10 flex items-center gap-2">
              ...
              {converting ? `Processing... ${progress}%` : `Convert to MP4`}
            </span>
          </button>
        </div>
      </div>
      ...
      <AnimatePresence>
        {result && !converting && (
          <motion.div
            ...
            className="glass-card rounded-2xl p-6 border-orange-500/20 text-center shadow-xl"
          >
            ...
            <div className="relative rounded-xl overflow-hidden bg-black/5 mb-6 max-h-[400px] flex items-center justify-center">
              <video
                src={result.url}
                controls
                className="max-h-[300px] relative z-10 object-contain mx-auto border border-border rounded-lg shadow-2xl"
              />
            </div>
            <button
              onClick={() => {
                const newName = file?.name.split(".")[0] + ".mp4";
